...
class Controller {
  ...
  async deleteAllDocs() {
    debug('deleteAllDocs');
    try { await this.model.deleteMany({}); } catch (e) { debug(e.message); return Promise.reject(e); }
  ...
  async createDocs(body) {
    debug('createDocs');
    let result;
    try { result = await this.model.create(body); } catch (e) { debug(e.message); return Promise.reject(e); } 
    debug(result);
